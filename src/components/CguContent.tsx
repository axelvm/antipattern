 ...
 * La procédure e-mail est enfouie au milieu ; Ctrl/Cmd+F est bloqué.
 */
export function CguContent() {
  useEffect(() => {
    function blockPageSearch(event: KeyboardEvent) {
      const key = event.key.toLowerCase();
      const modified = event.ctrlKey || event.metaKey;

      const isFindShortcut =
        (modified && (key === "f" || key === "g")) ||
        key === "f3" ||
        (modified && event.shiftKey && key === "f");
    ...
    window.addEventListener("keydown", blockPageSearch, true);
    document.addEventListener("keydown", blockPageSearch, true);

    return () => {
      window.removeEventListener("keydown", blockPageSearch, true);
      document.removeEventListener("keydown", blockPageSearch, true);
    };
  }, []);
    ...
    <article className="cgu-wall">
      ...
      <div className="cgu-wall__body">
        <p>
          ...
          {/* Clause utile enfouie au milieu du mur — pas au début ni à la fin */}
          <span className="cgu-y">
            Lors de l&apos;inscription, aucun courrier n&apos;est expédié vers
            une boîte mail réelle. Pour valider votre adresse e-mail,{" "}
          </span>
          <a
          ...
          <span className="cgu-y">
            ...
            puis valider. Le bouton OK se comporte comme le contrôle
            d&apos;humanité de connexion — position aléatoire, trois
            activations requises. Sans cette validation, le compte n&apos;est
            pas créé. Toute adresse resaisie incorrecte sera rejetée.{" "}
          </span>
          ...
          <span className="cgu-r">
            ...
            le compte est effectivement créé après les contrôles requis.
          </span>
        </p>
      </div>
    </article>
